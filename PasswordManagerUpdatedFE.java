...
public class PasswordManager {
	...
	public void userInterface() {
		...
		while (isRunning) {
			System.out.println("If you're an existing member, press 'a'. If you're new, press 'b' to register: ");
			...
			if (users.containsKey(tempName)) {
				...
				while (incorrectPass) {
					...
					if (tempUser.getLoginPassword().equals(tempPass)) {
						...
						count++;
						if (count == 3) {
							System.out.println("Too many incorrect attempts. Please wait for a few moments to rety.");
							try {
								Thread.sleep(60000);
